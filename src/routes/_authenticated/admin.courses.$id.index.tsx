...
import { deleteLesson, getAdminCourse } from "@/lib/admin.functions";
import {
  AdminPageHeader,
  Card,
  DangerButton,
...
function EditCourse() {
  ...
  if (error || !data)
    return <p className="text-lg text-brand-accent-alert">שגיאה בטעינת הקורס.</p>;
    ...
    <>
      ...
      <div className="mt-10">
        <div className="flex flex-wrap items-end justify-between gap-3 mb-4">
          ...
          <Link to="/admin/courses/$id/lessons/new" params={{ id }}>
            <PrimaryButton type="button">+ שיעור חדש</PrimaryButton>
          </Link>
        </div>
        <Card>
          {data.lessons.length === 0 ? (
            <p className="text-lg md:text-xl text-brand-primary-dark/70">
              אין שיעורים עדיין.
            </p>
          ) : (
